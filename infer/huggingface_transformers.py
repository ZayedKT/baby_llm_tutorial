""" 
This script downloads the model checkpoint (in this case facebook/opt-125m, one of the smallest language models on huggingface), and generates the output. Since the model is very small, the inference should be very fast (finishing in around a second). Feel free to explore other models on huggingface, and replace the model_name variable with your chosen one.
"""
...
def main():
    # Initialize model. The last parameter 'device_map="auto"' uses the accelerate library that we already installed
    ...
    prompt = "Question: What is the weather like in Abu Dhabi in general?\nAnswer: "
